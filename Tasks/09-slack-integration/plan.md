# Task 09: Slack Integration - Implementation Plan

## Overview
Implement 28 Slack tools using `slack_sdk.WebClient` with `asyncio.to_thread()`.

**Final state:** 224 tools total (196 existing + 28 new).

---

## Step 1: Dependencies & Configuration

### 1a. Add `slack-sdk` to `pyproject.toml`
Add to dependencies and pyright exclude.

### 1b. Add config to `config.py`
```python
# Slack
...
Create `src/mcp_toolbox/tools/slack_tool.py` — complete file:
...
def _get_client() -> WebClient:
    ...
    if not SLACK_BOT_TOKEN:
        raise ToolError(
            "SLACK_BOT_TOKEN not configured. Set it in your environment."
        )
...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def slack_get_channel_history(
        ...
        if oldest: kwargs["oldest"] = oldest
        if latest: kwargs["latest"] = latest
    ...
    async def slack_list_channels(
        ...
        return _success(200, data=channels, count=len(channels), next_cursor=_get_cursor(result))
    ...
    async def slack_list_channel_members(
        ...
        return _success(200, data=members, count=len(members), next_cursor=_get_cursor(result))
    ...
    async def slack_set_channel_purpose(channel: str, purpose: str) -> str:
        ...
        """
        result = await _call("conversations_setPurpose", channel=channel, purpose=purpose)
    ...
    async def slack_remove_reaction(channel: str, timestamp: str, name: str) -> str:
        ...
        Args:
            ...
            name: Emoji name
        """
        await _call("reactions_remove", channel=channel, timestamp=timestamp, name=name)
    ...
    async def slack_get_reactions(channel: str, timestamp: str) -> str:
        ...
        Args:
            ...
            timestamp: Message timestamp
        """
        result = await _call("reactions_get", channel=channel, timestamp=timestamp, full=True)
    ...
    async def slack_upload_file(
        ...
        if initial_comment: kwargs["initial_comment"] = initial_comment
...
Update `__init__.py` — add `slack_tool` to imports and `register_all_tools`.

---

## Step 4: Tests

Create `tests/test_slack_tool.py` with mocked WebClient. One test per tool + error tests. Uses same `_get_result_data` pattern. Fixture yields `(mcp, mock_client)` with `_slack_client` patched.

All 28 tools tested + 1 SlackApiError test + 1 missing token test = 30 tests.

---

## Step 5: Update test_server.py

Add all 28 Slack tool names. Total: `assert len(tools) == 224`.

---

## Step 6: Documentation & Validation

Update CLAUDE.md. Run `uv sync`, `pytest`, `ruff`, `pyright`.
...
| 2 | Tool module (complete file) | Step 1 |
| 3 | Registration | Step 2 |
| 4 | Tests | Steps 2-3 |
| 5 | test_server.py | Steps 2-3 |
| 6 | Docs & validation | Steps 1-5 |

---

## Risk Notes

- **`_call` uses `getattr`**: SDK method names use underscores (`chat_postMessage`) not dots. All names verified against slack_sdk source.
- **File upload**: `files_upload_v2` SDK method maps `file_path` tool param to `file` SDK kwarg.
- **DM pattern**: `slack_send_dm` calls `conversations_open` then `chat_postMessage` — two API calls per invocation.
- **Pagination**: All paginated tools include `next_cursor` in response.
